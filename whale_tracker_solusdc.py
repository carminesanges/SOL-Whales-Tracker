#!/usr/bin/env python3
# whale_tracker_solusdc.py
# v2.3 — SOL/USDC ONLY — bootstrap REST + WS live + non-blocking Telegram alerts
# English output, compact for CMD: Accumulation (green), Distribution (red), Neutral (yellow)

import asyncio, json, time, math, signal, os
...
BINANCE_WS_URL   = f"wss://stream.binance.com:9443/stream?streams={SYMBOL.lower()}@aggTrade"
...
BIG_TRADE_THRESHOLD_USDC = 50_000.0
K2_MULTIPLIER            = 3.0
# Adjusted for USDC: was 120_000.0 -> now 30_000.0
DELTA_WHALE_MIN_USDC     = 30_000.0

ALERT_DEBOUNCE_MS = 7000
# ============================================
...
def _format_k(value: float) -> str:
    ...
    if s >= 1_000_000: return f"{sign}{s//1_000_000}M"
    if s >= 1_000:     return f"{sign}{s//1_000}k"
    return f"{sign}{s}"
...
def format_short_line(result: Dict, show_mu_sigma: bool = False) -> str:
    ...
    if show_mu_sigma:
        mu = result.get('mu', 0.0); sigma = result.get('sigma', 0.0)
        core += f" | μ:{mu} σ:{sigma}"
    return core
...
class MarketState:
    """Manages trades and signals for SOL/USDC (bootstrap + live)."""
...
def bootstrap_state(state: MarketState):
    ...
    try:
        r = requests.get(BINANCE_REST_URL, params=params, timeout=5)
        r.raise_for_status()
        data = r.json()
    ...
    for t in trades:
        ...
        price = float(t.get("p", "0")); qty = float(t.get("q", "0"))
...
async def evaluator_loop(state: MarketState):
    """Periodically evaluates state, prints output, and sends Telegram alerts."""
    while True:
        now_ms = int(time.time() * 1000)
        result = state.evaluate()
...
async def websocket_loop(state: MarketState):
    ...
    while True:
        try:
            async with websockets.connect(BINANCE_WS_URL, ping_interval=15, ping_timeout=20, max_queue=2000) as ws:
...
async def main():
    print(f"{get_current_hms()} | {BOLD}Whale Accumulation Detector started for {SYMBOL}{RESET} | Window: {WINDOW_SEC}s | Poll: {POLL_SEC}s")
    print_legend()
    ...
    def _signal_handler(): stop_event.set()
